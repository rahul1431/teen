...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _changePhoto() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _user = {...?_user, 'avatar_url': res.data['avatar_url']}; });
  ...
  Future<void> _toggleBiometric(bool val) async {
    if (val) {
      ...
      if (creds == null) {
        if (mounted) AppSnackBar.show(context, 'Please logout and login again once to register biometrics.', error: true);
        return;
      }
      final canAuth = await _localAuth.canCheckBiometrics;
      if (!canAuth) {
        if (mounted) AppSnackBar.show(context, 'Biometrics not available on this device');
        return;
      }
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      _menuCard([
                        _menuItem(Icons.account_balance_rounded, locale.t('bank_details'),
                            subtitle: 'Add or update bank account',
                            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const BankDetailsPage()))),
                        _menuItem(Icons.receipt_long_rounded, locale.t('transaction_history'),
                      ...
                      _menuCard([
                        _menuItem(Icons.support_agent_rounded, locale.t('support'),
                      ...
                      _menuCard([
                        _menuItem(Icons.language_rounded, locale.t('language'),
                            ...
                            onTap: () => Navigator.push(context,
                                MaterialPageRoute(builder: (_) => const LanguageSelectionPage())).then((_) => setState(() {}))),
                        _menuToggle(
  ...
  Widget _menuToggle(IconData icon, String title, {
    String? subtitle, required bool value, required void Function(bool) onChanged,
  }) => Padding(
  ...
  Widget _buildAvatar() {
    ...
    final initial = ((_user?['username'] as String?)?.isNotEmpty == true
        ? _user!['username'][0] : 'P').toUpperCase();
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: AppColors.gold.withOpacity(0.35), blurRadius: 18, spreadRadius: 2)],
                    ...
                    child: avatarUrl != null
                        ? Image.network(avatarUrl, fit: BoxFit.cover,
                            errorBuilder: (_, __, ___) => Center(
                              child: Text(initial, style: const TextStyle(fontSize: 34, color: Colors.black, fontWeight: FontWeight.bold))))
                        : Center(child: Text(initial, style: const TextStyle(fontSize: 34, color: Colors.black, fontWeight: FontWeight.bold))),
                  ),
                ),
                Positioned(
                  bottom: 0, right: 0,
  ...
  Widget _buildKycBanner() {
    final kycColor = _kycApproved ? AppColors.green
        : _kycStatus == 'under_review' ? AppColors.orange
        : _kycStatus == 'rejected' ? AppColors.red : AppColors.orange;
    final kycIcon = _kycApproved ? Icons.verified_rounded
        : _kycStatus == 'under_review' ? Icons.access_time_rounded
        : _kycStatus == 'rejected' ? Icons.cancel_rounded : Icons.warning_amber_rounded;
    final kycTitle = _kycApproved ? 'KYC Verified'
        : _kycStatus == 'under_review' ? 'KYC Under Review'
        : _kycStatus == 'rejected' ? 'KYC Rejected — Resubmit' : 'Complete KYC Verification';
    final kycSub = _kycApproved
        ? 'Your identity is verified. No withdrawal limits.'
        : _kycStatus == 'under_review' ? 'Documents submitted — review within 24-48 hours'
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(kycTitle, style: TextStyle(fontWeight: FontWeight.bold, color: kycColor, fontSize: 13)),
            const SizedBox(height: 2),
            Text(kycSub, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
          ])),
  ...
  Widget _buildLogoutButton() => SizedBox(
    ...
    child: OutlinedButton.icon(
      ...
      label: Text(locale.t('logout'), style: const TextStyle(color: AppColors.red, fontWeight: FontWeight.bold)),
