...
/// Ludo game history — paginated list of settled Ludo games for the player,
/// backed by the generic `/api/wallet/game-history?game_type=ludo` endpoint
/// (same one the Teen Patti history page uses).
class LudoHistoryPage extends StatefulWidget {
  const LudoHistoryPage({super.key});

  @override
  State<LudoHistoryPage> createState() => _LudoHistoryPageState();
}

class _LudoHistoryPageState extends State<LudoHistoryPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.dio.get('/api/wallet/game-history',
          queryParameters: {'game_type': 'ludo', 'limit': 20, 'offset': 0});
  ...
  Future<void> _loadMore() async {
    ...
    try {
      final res = await _api.dio.get('/api/wallet/game-history',
          queryParameters: {'game_type': 'ludo', 'limit': 20, 'offset': _offset});
      final rows = (res.data as List?) ?? [];
      setState(() {
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                won ? 'You Won!' : tied ? 'Draw' : 'You Lost',
                style: TextStyle(
                    color: won ? AppColors.gold : tied ? Colors.white70 : AppColors.red,
