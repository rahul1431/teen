...
/// Friends table lobby — create a private Teen Patti table and share its
...
class _TeenPattiFriendsPageState extends State<TeenPattiFriendsPage> {
  ...
  Future<void> _init() async {
    ...
    _closedSub = _socket.on('private:closed').listen((data) {
      if (!mounted) return;
      final reason = (data is Map ? data['reason']?.toString() : null) ?? 'Table closed';
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(reason), backgroundColor: AppColors.red));
      setState(() => _lobby = null);
    });

    // Host pressed Start → the server deals and pushes room:joined to everyone.
    ...
    _errorSub = _socket.on(SocketEvents.errorEvent).listen((data) {
      ...
      final msg = (data is Map ? data['message'] as String? : null) ?? 'Something went wrong';
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildLobby() {
    final maxPlayers = (_lobby?['max_players'] as num?)?.toInt() ?? 6;
      ...
      children: [
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                  colors: [Color(0xFF3A2230), Color(0xFF24121C)]),
        ...
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              final p = _players[i];
              final isHost = p['user_id']?.toString() == _lobby?['host_id']?.toString();
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: isMe ? Border.all(color: AppColors.gold.withValues(alpha: 0.6)) : null,
                ),
                child: Row(
                  children: [
                    CircleAvatar(
