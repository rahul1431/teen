...
class _DailyBonusPageState extends State<DailyBonusPage>
  ...
  Future<void> _claim() async {
    ...
    try {
      final res =
          await ApiClient().dio.post('/api/users/daily-bonus/claim', data: {});
      ...
      if (mounted) {
        final msg =
            (e as dynamic).response?.data?['error'] ?? 'Could not claim bonus';
        ...
        child: ScaleTransition(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: AppColors.green.withValues(alpha: 0.5), width: 2),
              boxShadow: [
                BoxShadow(
              ...
              children: [
                ...
                Text('+${formatCurrency(amount)}',
                    style: const TextStyle(
                      ...
                      shadows: [
                        Shadow(color: Color(0x6600C853), blurRadius: 16)
                      ],
                    )),
  ...
  double get _totalEarned =>
      double.tryParse(_data?['total_earned']?.toString() ?? '0') ?? 0;
  ...
  Widget _buildStatsRow() => Row(
        children: [
          Expanded(
              child: _statCard('🏆', 'Best Streak', '$_longestStreak days',
                  const Color(0xFF4A2E00), AppColors.gold)),
          const SizedBox(width: 10),
          Expanded(
              child: _statCard(
                  '💰',
                  'Total Earned',
                  formatCurrency(_totalEarned),
                  const Color(0xFF1A4C2E),
                  AppColors.green)),
          const SizedBox(width: 10),
          Expanded(
              child: _statCard('📅', 'Days Claimed', '$_totalClaimed',
                  const Color(0xFF1E2A4A), AppColors.blue)),
        ],
      );
          ...
          children: [
            ...
            _schedule.isEmpty
                ...
                : Wrap(
                    ...
                    runSpacing: 10,
                    children: _schedule
                        .map((d) => _dayTile(d as Map<String, dynamic>))
                        .toList(),
                  ),
          ],
        ),
  ...
  Widget _dayTile(Map<String, dynamic> day) {
    ...
    if (isSpecial) {
      ...
      bgColor = AppColors.gold.withValues(
          alpha: isToday
              ? 0.25
              : isClaimed
                  ? 0.1
  ...
  Widget _buildClaimButton() {
    final amount =
        double.tryParse(_todayBonus['bonus_amount']?.toString() ?? '0') ?? 0;
