    ...
    app.post('/telegram/webhook', async (req, reply) => {
      ...
      try {
        ...
        const existing = await db.query(
          `SELECT user_id FROM user_telegram_links WHERE telegram_user_id = $1`,
          [telegramUserId],
        )
        if (existing.rowCount && existing.rows[0].user_id !== payload.sub) {
          await callTelegramApi('sendMessage', {
            chat_id: message.chat.id,
            text: 'This Telegram account is already linked to a different account. Contact support if you believe this is a mistake.',
          })
          return reply.send({ ok: true })
        }
