...
export async function debitStake(opts: {
  userId: string; amount: number; referenceId: string; idempotencyKey: string; description?: string
}): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(`${WALLET_URL}/internal/wallet/debit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-internal-key': KEY },
      body: JSON.stringify({ user_id: opts.userId, amount: opts.amount, reference_id: opts.referenceId, idempotency_key: opts.idempotencyKey, description: opts.description }),
    })
    if (!res.ok) {
