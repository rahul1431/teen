...
describe('BotTrainingConfigRepository - tiered_hard_wins', () => {
  it('default config includes fallbackStrategy = lifetime_winrate', async () => {
    ...
    const config = await repo.getConfig('ludo')
  ...
  it('accepts tiered_hard_wins as a valid strategy', async () => {
    ...
    const config = await repo.getConfig('ludo')
    await expect(
      repo.updateConfig('ludo', { ...config, strategy: 'tiered_hard_wins', fallbackStrategy: 'rotation' })
  ...
  it('rejects an invalid fallbackStrategy value', async () => {
    ...
    const config = await repo.getConfig('ludo')
    await expect(
      repo.updateConfig('ludo', { ...config, fallbackStrategy: 'not_a_real_strategy' as any })
  ...
  it('rejects an invalid strategy value', async () => {
    ...
    const config = await repo.getConfig('ludo')
    await expect(
      repo.updateConfig('ludo', { ...config, strategy: 'not_a_real_strategy' as any })
