...
| 023 | `023_app_versions.sql` | `app_versions` (`SERIAL` PK, unique `version_code`), default `download_url` pointing at `game.myonlinejoker.com/downloads/app-release.apk`. | The app-level bug this default URL was connected to (every version sharing one download URL) was fixed 2026-07-28 — see `docs/admin-panel/app-update/overview.md`. |
...
| 029 | `029_tip_dealer_drop_gifts.sql` | `ALTER TYPE txn_type_enum ADD VALUE 'tip_dealer'`; **`DROP TABLE IF EXISTS game_gifts`**. | **Destructive** — see Notable Risks. Enables the dealer-tip feature (`docs/games/teen-patti/backend.md`), whose idempotency-key bug was fixed 2026-07-28. |
...
2. The runner-level root cause behind finding #1 and the already-fixed 012/027 incident: no migration file's mid-file SQL failure was ever detected by `migrate.sh`, so broken migrations got recorded as successfully applied. Fixed 2026-07-28 — both `psql` invocations in `migrate.sh` now pass `-v ON_ERROR_STOP=1`, so a failing statement produces a real non-zero exit code and the existing abort-on-failure logic actually triggers.
