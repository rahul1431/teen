...
`PORT` defaults to `3010` if unset (`main.go:868-871`); the process binds `http.ListenAndServe(":"+port, mux)` — **all interfaces, not `127.0.0.1`** — see `backend.md` and `docs/Bugs/teen-patti-engine-no-auth-or-turn-enforcement.md` for why that matters given there is no request authentication anywhere in this file.
