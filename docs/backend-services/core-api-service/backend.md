...
- `GET /admin/bank-details` / `PATCH /admin/bank-details/:userId/verify` — a byte-for-byte, zero-auth duplicate of the properly-secured `admin-service` routes used to live here (`users.ts`). Removed 2026-07-29 rather than patched with auth, since admin-service already owns this feature and nothing called this copy.

## Leaderboard — `plugins/leaderboard.ts`

`GET /leaderboard/:gameType` (live Postgres aggregate, accepts player JWT or `x-internal-key`) and a dead `POST /internal/leaderboard/update` (writes to Redis sorted sets nothing else reads). Full detail: `docs/app/leaderboard/backend.md`. The home screen's "Top 3 Win Big" promo used to have no backend payout behind it at all — fixed 2026-07-29: `modules/leaderboard/{rewards,scheduler}.ts` now pays yesterday's top-3 Teen Patti and Aviator finishers (₹500/₹300/₹200) to their bonus wallet once daily, recorded in the new `leaderboard_rewards` table.
