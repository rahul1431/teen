...
4. On `close`, `hub.remove(conn)` runs, then (fixed 2026-07-28) `matchmaking.leaveQueueOnDisconnect(userId)` dequeues whatever matchmaking search this user was most recently in — but only once `hub.hasUser(userId)` confirms this was their *last* live connection, so a still-connected second device isn't kicked out of a shared queue. Previously a dropped connection while queued left a phantom entry indefinitely, since only the explicit `leave_matchmaking` event ever called `leaveQueue()`.
...
## Completed-room reconcile + Ludo queue drain (added 2026-07-28, same `SWEEP_MS` cadence)

A failed per-player `consumeLockedFunds` during `POST /internal/wallet/settle-game` (non-fatal per player, collected into `consume_errors` that nothing read) used to be able to strand that player's stake in `locked_balance` forever: the room finishes `'completed'`, the winner is still paid in full, and the idle-room sweep above only ever scans `'waiting'`/`'active'` rooms. Fixed 2026-07-28.

- **`reconcileCompletedRooms()`**: `SELECT id, game_type FROM game_rooms WHERE status = 'completed' AND ended_at < NOW() - INTERVAL '2 minutes' AND ended_at > NOW() - INTERVAL '24 hours'` (the 2-minute floor avoids racing an in-flight settle-game call; the 24h ceiling bounds the query). For each room, `reconcileRoom()` checks every participant with `entry_fee_deducted > 0` for a missing `consume:<roomId>:<userId>` transaction and retries `POST /internal/wallet/consume` for each one found. Logs an event into `watchdog_events` with `action = 'reconciled_stranded_consume'` — a distinct action from `'reaped'` sharing the same table (the admin Watchdog tab and its stats query were updated to not conflate "money collected" with "money refunded", since they're opposite semantics on the same `refunds`/`total_refunded` columns).
- **`drainLudoReconcileQueue()`**: drains `ludo:reconcile:failed`, a Redis list three call sites (two in `matchmaking.ts`, one in `services/game-engines/ludo/src/index.ts`) already pushed whole-call settle-game failures into but that nothing ever read before this fix. Re-derives fresh `game_participants` data at drain time rather than trusting the queued snapshot, retries the full `settle-game` call (safe via `idempotency_key: settle_<roomId>`), and re-queues on failure (capped batch of 20/sweep, stops the cycle on first failure to avoid hot-looping a persistently-broken entry). Drops (rather than re-queues) any entry whose `room_id` isn't a valid UUID — hit live immediately after deploy from a manually-pushed smoke-test entry that would otherwise have retried forever.

Both `matchmaking.ts` settle-game call sites (`handleGameEnd`, `handleLudoEnd`) now also log loudly when a *successful* settle-game response still carries `consume_errors` — previously discarded silently; the reconcile sweep above is what actually fixes the underlying stranded lock, this is just visibility.
...
`driveLudoBots()` loops (bounded to 400 iterations as a runaway guard) calling `<LUDO_ENGINE_URL>/bot-turn` with a fixed 1200ms pacing sleep between calls, for as long as the current turn belongs to a bot. The moment it finds a human's turn, it calls `scheduleLudoAfkTimer(roomId, turnIdx)` and returns.

`LUDO_TURN_TIMEOUT_MS = 30000` (30s). If it fires, `autoPlayIdleLudoTurn()` re-checks the authoritative Redis deadline and current state (the human may have just acted, possibly via a different gateway instance — see below), and if still their turn, plays the *minimum* legal action for them: `move_token` with the first entry in `movable_tokens` if a move is owed, otherwise a full `/bot-turn` call (which rolls **and** auto-picks/plays a token via the engine's own bot AI if a move follows the roll — the inline comment describing this as "no strategy needed" undersells that the auto-played move, when one happens, actually goes through the same `chooseBotToken()` difficulty-aware logic real bots use, not a naive first-legal-move pick). The player is told `'You took too long — your turn was played automatically.'` via a direct `error`-channel message to just that user.

### Teen Patti's equivalent human AFK timer

Teen Patti has its own equivalent, `scheduleTeenPattiAfkTimer`/`autoFoldIdleTeenPattiTurn` (`TEEN_PATTI_TURN_TIMEOUT_MS = 30000`, live since 2026-07-12 — an earlier pass of this doc incorrectly claimed no such timer existed). `scheduleBotTurn` arms it the moment it finds the current turn belongs to a connected human rather than a bot, instead of leaving the table with no server-side path forward. On expiry it folds the idle player (always legal, no wallet call, so it can never double-charge or move money the player wouldn't have moved themselves) and re-schedules the next turn.

Both AFK timers share the same cross-instance hazard and the same fix: `game-gateway` runs as 3 PM2 processes behind nginx, hashed by player token (not by room), so a turn-holding action processed on a *different* instance than the one that armed the timer would otherwise go unnoticed — confirmed live for Teen Patti (a player who sent `'see'` via one instance at T+20s and was about to `'call'` via another at T+45s got wrongly folded at T+40s by the original instance's blind timer). Both timers now keep their deadline authoritative in Redis (`tp:afk:<roomId>` / `ludo:afk:<roomId>`, mirroring `GameWatchdog`'s liveness-key pattern): every instance's local `setTimeout` is just a wake-up that re-reads Redis before acting, and re-arming (`clearTeenPattiAfkTimer`/`clearLudoAfkTimer`, called the instant a real action is *received*, before the slow wallet-lock/engine round trip) invalidates any timer racing against it on another instance.

If neither the gateway process handling a room nor a stuck player's own client can unstick a hand — e.g. the gateway itself loses connectivity to an engine — the room-level 15-minute `GameWatchdog` (refunds *everyone* at the table, not just the stuck player) or an admin's internal `force-action` call remain the fallback for both games.
...
The engine itself (`services/game-engines/teen-patti/main.go`) used to have no per-room lock or mutex around its `/action` handler — a plain `redis.Get` → unmarshal → mutate → `redis.Set` with nothing serializing two concurrent calls for the same `room_id`, reachable in practice because the gateway can issue two calls for the same room close together (a scheduled bot-turn timer firing at the same moment a human sends an out-of-turn `see`/`sideshow_accept`/`sideshow_reject`, both explicitly allowed out-of-turn, `index.ts:541`). Fixed 2026-07-29: `main.go` now has its own `withRoomLock()` (Redis `SET NX PX` mutex keyed `tp:lock:<roomId>`) wrapping `processAction`, mirroring the Ludo engine's equivalent below.
