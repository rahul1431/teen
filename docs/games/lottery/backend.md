...
**Severity: High, fixed 2026-07-28.** `lottery_tickets` had no `UNIQUE(draw_id, ticket_number)` constraint or index. `/lottery/buy`'s protection against double-selling a number was a plain `SELECT` pre-check (`betting.ts:96`) followed by an `INSERT`, with a `catch` block specifically written to handle a `23505` unique-violation (`:106-111`) that couldn't be thrown by that schema — there was nothing to violate. Two requests for the same `draw_id`+`ticket_number` that raced past the pre-check within the same window (two different users tapping "Buy" on the same quick-picked number at nearly the same instant, or a single client's request being retried by a proxy/timeout) could both debit successfully and both insert successfully — if that number was later declared a winner, `settleLottery`'s unscoped `UPDATE ... WHERE draw_id=$2 AND ticket_number=$3` would have marked and paid **both** rows the full declared prize. A `UNIQUE(draw_id, ticket_number)` constraint (checked against live data first — no existing duplicates) now makes the already-correct `23505`-handling catch block (refunds the losing attempt's debit) actually reachable.
...
Every idempotency key on this feature — `lottery_buy_<ticketId>` (`betting.ts:100`), `lottery_payout_<ticketId>` (`helpers/lottery.ts:67`), `lottery_refund_<ticketId>` (`betting.ts:322`) — is built from a UUID minted server-side inside the handler (`crypto.randomUUID()`, `betting.ts:99`), not something the caller supplies and can echo back on a retry. This was deliberately **not** changed to a deterministic key as part of the 2026-07-28 fix above: `debitStake()` has no way to signal "this was a deduped no-op" vs. "freshly debited," so a deterministic key (e.g. derived from `draw_id`+`ticket_number`+`user_id`) would make a legitimate retry-after-success double-credit via the refund path below, rather than being a true no-op. Instead, the unique constraint alone resolves this correctly via compensation: a dropped-response retry of the exact same "buy ticket 1234" request produces a second, differently-keyed debit, the second `INSERT` hits the new constraint (`23505`), and the existing catch block refunds that second attempt's own debit — a transient debit-then-refund round trip rather than a clean idempotent no-op, but not a permanent double-charge either.
