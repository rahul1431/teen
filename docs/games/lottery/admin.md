...
The stale header comment above these routes ("write actions proxy to the **betting-service** internal endpoints," `services/admin-service/src/index.ts:1458-1460`) is a leftover from before the standalone `betting-service` was folded into `core-api-service` — `BETTING_URL`'s default port `3012` is not where anything listens today (`core-api-service` runs on port `3001` per `ecosystem.config.js:32`); the routing only actually works in production because `services/admin-service/.env` sets `BETTING_SERVICE_URL` to `core-api-service`'s real address. This is the same class of stale-URL-default issue already filed for notifications (`docs/Bugs/admin-notification-betting-urls-point-to-dead-ports.md`) but for a different env var and a different callee — not re-filed as a separate bug here since the `.env` override makes it a latent rather than live gap (same caveat that doc gives for its own case), but worth knowing if `BETTING_SERVICE_URL` is ever unset in a fresh environment.
