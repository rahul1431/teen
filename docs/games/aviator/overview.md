...
- **Port**: `PORT=3005` (`services/game-engines/aviator/.env:1`), matching all three committed Nginx configs' `aviator_backend`/`/ws/aviator` upstream and confirmed live on the VPS (`ss -tlnp` shows the `teen-aviator` PM2 process bound to 3005). The source falls back to the same value if `PORT` is ever unset (`parseInt(process.env.PORT || '3005')`, `src/index.ts:597`). An earlier documentation pass incorrectly concluded this port was "misrouted" to `monitoring-service` (fixed 2026-07-28) — that was based on `monitoring-service`'s *committed `.env.example`* also claiming `3005`, without checking that its actual live `.env` (and now its `.env.example`, corrected in the same pass) uses **3017** instead, specifically to avoid this collision.
