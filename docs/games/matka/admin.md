...
The admin write path (declare, market create, market delete) is further affected by the already-filed `docs/Bugs/admin-notification-betting-urls-point-to-dead-ports.md`: `callBetting()`'s `BETTING_URL` fallback (`services/admin-service/src/index.ts:1461`, `http://127.0.0.1:3012`) points at the old pre-merge standalone `betting-service`'s port, which nothing listens on since Matka/Lottery/Cricket were folded into `core-api-service` (port 3001) — and `services/admin-service/.env`'s explicit `BETTING_SERVICE_URL` setting matches that same wrong port. Per that bug's findings, **declaring a Matka result from the admin panel currently fails with a 500** in this deployment, independent of any of the correctness issues documented above (market create/delete/list read routes query Postgres directly from `admin-service` and are unaffected, since they don't go through `callBetting()`).
