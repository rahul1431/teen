...
`matchmaking.ts`'s Teen Patti branch of `startGame` used to call the engine's `/start` exactly once and silently fall back to a cardless `fallbackState` on any failure — no retry, no room teardown — while Ludo's equivalent branch already retried once and unlocked/cancelled on continued failure. Teen Patti now mirrors that: it retries the `/start` call once after a 1.5s sleep, and if it still fails, unlocks every locked stake, marks the room `cancelled`, and notifies real players immediately instead of leaving a table that 404s forever on every `/action` call.
